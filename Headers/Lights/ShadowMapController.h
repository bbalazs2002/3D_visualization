...
private:
	...
	// --- SSBO ---
	inline static GLuint lightSpaceMatSSBOID = 0;
	...
	static void AllocateLightSpaceSSBO() {
		if (lightSpaceMatSSBOID > 0) {
			glDeleteBuffers(1, &lightSpaceMatSSBOID);
		}
		size_t size = CountlightSpaceMatrices();
		if (size > 0) {
			glCreateBuffers(1, &lightSpaceMatSSBOID);
			glBindBuffer(GL_SHADER_STORAGE_BUFFER, lightSpaceMatSSBOID);
			glNamedBufferStorage(lightSpaceMatSSBOID, sizeof(glm::mat4) * size, nullptr, GL_MAP_WRITE_BIT | GL_DYNAMIC_STORAGE_BIT);
		}
	}
...
public:
	...
	static size_t CountlightSpaceMatrices() {
		return handles2D.size() + 6 * handlesCube.size();
	}

	// --- Map light space SSBO for writing ---
	static bool Map2DLightSpaceSSBO(size_t index, GLfloat*& buffer) {
		glBindBuffer(GL_SHADER_STORAGE_BUFFER, lightSpaceMatSSBOID);
		buffer = (GLfloat*)glMapBufferRange(GL_SHADER_STORAGE_BUFFER, index * sizeof(glm::mat4), sizeof(glm::mat4), GL_MAP_WRITE_BIT | GL_MAP_UNSYNCHRONIZED_BIT);
		if (!buffer) {
			Log::errorToConsole("Failed to map SSBO for writing 2d light space matrix");
			glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
			return false;
		}
		return true;
	}
	static bool MapCubeLightSpaceSSBO(size_t index, GLfloat*& buffer) {
		glBindBuffer(GL_SHADER_STORAGE_BUFFER, lightSpaceMatSSBOID);
		buffer = (GLfloat*)glMapBufferRange(GL_SHADER_STORAGE_BUFFER, index * sizeof(glm::mat4), sizeof(glm::mat4) * 6, GL_MAP_WRITE_BIT | GL_MAP_UNSYNCHRONIZED_BIT);
		if (!buffer) {
			Log::errorToConsole("Failed to map SSBO for writing cube light space matrix");
			glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
			return false;
		}
		return true;
	}
	// --- Unmap light space SSBO after writing ---
	static void UnmapLightSpaceSSBO() {
		glUnmapBuffer(GL_SHADER_STORAGE_BUFFER);
		glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
	}

	// --- Bind lightspace SSBO ---
	static void BindLightSpaceSSBO(GLuint bindingPoint) {
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, bindingPoint, lightSpaceMatSSBOID);
	}
	...
	static void Release2DLayer(GLuint index) {
	...
	static void ReleaseCubeLayer(GLuint index) {
	...
	// --- Prepare for writing ---
	static void Bind2DFBO(GLuint layerIndex, bool clear = true) {
		glBindFramebuffer(GL_FRAMEBUFFER, fbo2DArrayID);
	...
	static void BindCubeFaceToFBO(GLuint cubeLayerIndex, GLuint faceIndex, bool clear = true) {
		if (textureCubeArrayID > 0) {
	...
	static void UnbindFBO() {
		if (defViewport[2] > 0 && defViewport[3] > 0) {
	...
	static void UnbindTextures() {
		if (last2DUnit > 0) {
			glActiveTexture(GL_TEXTURE0 + last2DUnit);
			glBindTexture(GL_TEXTURE_2D_ARRAY, 0);
			last2DUnit = 0;
		}
		if (lastCubeUnit > 0) {
			glActiveTexture(GL_TEXTURE0 + lastCubeUnit);
			glBindTexture(GL_TEXTURE_CUBE_MAP_ARRAY, 0);
			lastCubeUnit = 0;
		}
	}
